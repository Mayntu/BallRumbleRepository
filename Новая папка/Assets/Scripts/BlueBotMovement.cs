...
public class BlueBotMovement : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (isBot)
        {
            ...
            {
                if (ball.GetComponent<PlayerThrowBall>().Player.tag == "BluePlayer")
                {
                    AI_Agent.SetDestination(ball.transform.position);
                    animator.SetFloat("speed", 0.4f);
                }
                else if (ball.GetComponent<PlayerThrowBall>().Player.tag == "RedPlayer")
                {
                    AI_Agent.SetDestination(ball.transform.position);
                    animator.SetFloat("speed", 0.4f);
                }
